...
Forecasts <- forecastsNOAA(stn = NOAA_STATIONS, probCut = 30, numbDays = 5, PrintDataTables = PRINT_TABLES)
...
yesterday <- today-1
if (DS[length(DS)] < yesterday){   # what to do in case the model didn't run for 1-2 days
	source("../Functions/Past72hrData.r") 
	Obs3days <- Past72hrData(NOAAstn = NOAA_STATIONS)
	cat("\n\nNOTE: Using NOAA observations for missing days\n") 
	if (DS[length(DS)] < (Obs3days$Date[1]-1)) {  ##  Print out a warning if still missing dates
		cat("\n\nPROBLEM: missing data! Between:\n") 
		print(DS[length(DS)])
		print(Obs3days$Date[1])
		cat("\n\n")
		st <- 1			## even is missing dates, we will continue with a gap in data (problematic for snow melt)
	} else   st <- which(Obs3days$Date > DS[length(DS)])[1]
	DS <- c(DS[1:max(which(DS==(Obs3days$Date[st]-1)), max(which(DS<Obs3days$Date[st])))], as.Date(Obs3days$Date[st:3]))
	Pmm <- c(Pmm[1:max(which(DS==(Obs3days$Date[st]-1)), max(which(DS<Obs3days$Date[st])))], Obs3days$P_mm[st:3]) 	# Precip, mm
	TxC <- c(TxC[1:max(which(DS==(Obs3days$Date[st]-1)), max(which(DS<Obs3days$Date[st])))], Obs3days$Tx_C[st:3]) 	# Daily Max Temp, C
	TnC <- c(TnC[1:max(which(DS==(Obs3days$Date[st]-1)), max(which(DS<Obs3days$Date[st])))], Obs3days$Tn_C[st:3]) 
	opt = "Full Run"
} else {   # If no missing data : NORMAL OPTION
...
n <-  nrow(TodaysMR)  # length(DS) + 3    #
TodaysMR$MaxWetClass[(n-3):n]/10  # as a percentage (assuming 10% wetness classes)	
o1 <- paste("var satPercent = ", TodaysMR$MaxWetClass[(n-DAYS_PREDICTION)]*10, " ;", sep="")
o2 <- paste("var satPercentTomorrow = ", TodaysMR$MaxWetClass[(n-DAYS_PREDICTION+1)]*10, " ;", sep="")
o3 <- paste("var satPercentDay2 = ", TodaysMR$MaxWetClass[(n-DAYS_PREDICTION+2)]*10, " ;", sep="")
o4 <- paste("var satPercentDay3 = ", TodaysMR$MaxWetClass[n-DAYS_PREDICTION+3]*10, " ;", sep="")
o5 <- paste("var satPercentDay4 = ", TodaysMR$MaxWetClass[n-DAYS_PREDICTION+4]*10, " ;", sep="")
o6 <- paste("var satPercentDay5 = ", TodaysMR$MaxWetClass[n-DAYS_PREDICTION+5]*10, " ;", sep="")
o7 <- paste("var satPercentMax = ", max(TodaysMR$MaxWetClass[(n-DAYS_PREDICTION):n]*10), " ;", sep="")
if (COMPUTER == "server" ){ 
 write.table(rbind(o1,o2,o3,o4,o7),file=paste("/var/www/", WSH, "/sat_percent.js", sep=""), quote=FALSE, col.names=FALSE, row.names=FALSE)
 } else  write.table(rbind(o1,o2,o3,o4,o7),file="sat_percent.js", quote=FALSE, col.names=FALSE, row.names=FALSE)
...
library(rjson)
if (COMPUTER == "server" ){ 
cat(paste("var t = ", toJSON(data.frame(LastRun,Date,Label,Today, Tomorrow, DayAfter, D3))), file=paste("/var/www/", WSH,"/table.jsonp", sep=""))
...
if (opt == "PrevRun"){
	...
	PrevRun <- TodaysMR[1:which(TodaysMR$Date==today),]
	PrevSM <- SM[1:which(SM$Date==today),]
}
...
ID <- c(as.character(today),PastDay$P_mm_NOAA, PastDay$P_mm_coop, Pred_P, PastDay$Tx, Pred_Tx, PastDay$Tn, Pred_Tn, Pred_Prob, as.character(PastDay$CoopStnsReporting))
...
## Print graph:
	if (COMPUTER == "server" ){ 
	jpeg(paste("/var/www/", WSH,"/ModelResults.jpeg", sep=""), width=12, height=6, units="in", res=300)		
	} else 	jpeg("ModelResults.jpeg", width=12, height=6, units="in", res=300) # pdf(file="ModelResults.pdf", width=12, height=6)
	plot(c(PrevRun$Date[Amod:Bmod],seq(today+1, today+DAYS_PREDICTION, by=1)), c(PrevRun$modeled_flow[Amod:Bmod],TodaysMR$modeled_flow[which(TodaysMR$Date>today)]), col="red", type="l", ylim=c(0, max(ObsFlow[Aobs:Bobs],c(PrevRun$modeled_flow[Amod:Bmod],TodaysMR$modeled_flow[which(TodaysMR$Date>today)]), na.rm=TRUE)), ylab="Flow depth (mm/d)", xlab="Date", lwd=2, lty=2)
	...
	legend("topleft", legend = c("USGS gage", "Modeled Flow", "Forecast"), lwd=c(NA,2.5,2), lty=c(NA,1,2),pch=c(1,NA,NA), col=c("black", "blue", "red"))
	dev.off()
}


##  Just for checking output: currently, can deleate:
if (b=="OwascoLake"){
	print(tail(TodaysMR))
}
