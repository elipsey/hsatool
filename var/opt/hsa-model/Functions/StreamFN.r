...
#####################################################
## depth (mm)
#rec_coef<-params[1]		## Zach:  0.07
#Se_avg<-params[2]			##  Zach:  140
#Ia_coef<-params[3]		## usually 0.05 or 0.2  Can be as low as 0.01
###################SW_coef<-params[4]		##	Fraction of soil water contributing to lateral unsaturated flow to baseflow
#SAT_coef<-params[5]		## The percent of water above FC, below saturation, that will drain in one day
###################seep_coef<-params[6]		##	deep seepage of groundwater (lost from system)
params_names<-c("rec_coef", "Se_avg", "Ia_coef", "SW_coef", "SAT_coef", "seep_coef")
...
StreamflowFN<-function(rec_coef=0.08,Se_avg=140, Ia_coef=0.05, AWC=AWCinput, SAT=Porosity,
latitudeDegrees=42.38, albedo=0.2, aspect=0, P, dateSeries, 
Tmax, Tmin, shift=1, SW1=NULL, BF1=1, PETcap=5, percentImpervious=0,SAT_coef=0.3, no_wet_class=10,
PreviousOutput=NULL){
# Parameters
...
# Option 3 - use the coefficients generated from Observed Streamflow   #### Might want to include these in parameters
	...
	rain_snowmelt<-P
	Streamflow<-data.frame(dateSeries, rain_snowmelt, modeled_flow, baseflow, OverlandFlow, ShallowInterflow, totQ, 
		deltaP, Pe, Se, Ia, SoilWater, quickflow_combined, impervRunoff, excess, Tmax, Tmin, Q2, MaxWetClass)[OutStart:length(P),] 

	if (nrow(Streamflow)==1){
		return (c(as.character(dateSeries[length(P)]), rain_snowmelt[length(P)], modeled_flow[length(P)], baseflow[length(P)], 
		OverlandFlow[length(P)], ShallowInterflow[length(P)], totQ[length(P)], 
		deltaP[length(P)], Pe[length(P)], Se[length(P)], Ia[length(P)], SoilWater[length(P)], quickflow_combined[length(P)], 
		impervRunoff[length(P)], excess[length(P)], Tmax[length(P)], Tmin[length(P)], Q2[length(P)], MaxWetClass[length(P)]))
